...
מעל 1250px מוצג סרגל הקישורים הרגיל. מתחתיו הוא מתחלף בכפתור תפריט
(`app/mobile-menu.tsx`), כי שבעה יעדים לא נכנסים לרוחב של טלפון. שתי הגרסאות
נבנות מאותו מערך `navItems` ב־`site-page.tsx`, כך שהן לא יכולות להיפרד.

התפריט מטפל בכל מה שנדרש מרכיב Disclosure: `aria-expanded` על הכפתור,
`aria-controls` שמצביע על הפאנל, Escape שסוגר ומחזיר את הפוקוס לכפתור, Tab
שנשאר בתוך הפאנל, לחיצה בחוץ שסוגרת, וגלילה נעולה מאחור. הפאנל לא נמצא ב־DOM
כשהוא סגור, ולכן אי אפשר להגיע לקישורים שלו ב־Tab. יש בדיקות e2e לכל אחד
מהסעיפים האלה.

## בלוג ושאלות נפוצות

הפוסטים יושבים ב־`lib/blog.ts`, מערך אחד לכל שפה. ה־slug משותף לשתי השפות כדי
ש־`/blog/x` ו־`/en/blog/x` יהיו אותו מאמר, ובדיקת יחידה נכשלת אם רשימות ה־slug
מתפצלות. הוספת פוסט היא הוספת אובייקט בשני המערכים — הנתיבים, ה־sitemap ורשימת
הפוסטים בעמוד הבית נגזרים מהמערך ולא צריך לגעת בהם.

השאלות הנפוצות יושבות ב־`content.faq.items` ומוצגות עם `<details>` — הפתיחה,
הסגירה והמקלדת מגיעות מהדפדפן ולא מקוד שלנו. אותן שאלות נשלחות גם כ־`FAQPage`
ב־JSON-LD, ובדיקת e2e משווה בין מה שהעמוד מציג לבין מה שהוא מפרסם לגוגל.
...
7. **אישור פוסטי הבלוג** — שלושת הפוסטים הראשונים נכתבו כטיוטה ומפורסמים תחת
   שמך. כדאי לקרוא ולאשר לפני שהאתר עולה לאינדקס.
8. **תשובת בעלות על הקוד** — התשובה בשאלות הנפוצות אומרת שהקוד עובר ללקוח
   בסיום. אם זה לא התנאי שלך, יש לעדכן את `content.faq.items`.
